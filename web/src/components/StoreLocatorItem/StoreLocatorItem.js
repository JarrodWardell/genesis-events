...
const StoreLocatorItem = ({ store, showDistance = true, isGoogleInitialized = false }) => {
    ...
    const loadData = async () => {
      ...
      service.findPlaceFromQuery({
        ...
        service.getDetails({
          ...
          fields: ['name', 'geometry', 'formatted_address', 'photos', 'opening_hours', 'place_id', 'website'],
        }, (place, status) => {
      ...
      }
      {
        googleStoreDetails.website && (
          <div className="flex mt-4">
            ...
            <p className="font-bold">Website: </p> <a className="ml-1" href={googleStoreDetails.website}> {googleStoreDetails.website}</a>
          </div>
        )
      }
  </Link>
